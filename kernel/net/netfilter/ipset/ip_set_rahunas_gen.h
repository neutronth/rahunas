...
#ifndef _IP_SET_RAHUNAS_GEN_H
#define _IP_SET_RAHUNAS_GEN_H
...
#define mtype_timeout_notify		IPSET_TOKEN(MTYPE, _timeout_notify)
...
#define mtype_deferred_gc		IPSET_TOKEN(MTYPE, _deferred_gc)
...
/* The workqueue structure */
struct htype_workqueue {
  struct workqueue_struct *handle;
...
struct htype {
  ...
  struct htype_workqueue wq; /* deferred work (expired check) */
...
mtype_destroy(struct ip_set *set)
{
  ...
  if (h->wq.handle) {
    flush_workqueue(h->wq.handle);
    destroy_workqueue(h->wq.handle);
  }
...
static void
mtype_deferred_gc(struct work_struct *work)
{
  struct htype_workqueue *wq = container_of(work, struct htype_workqueue, work);
  struct ip_set *set = wq->set;
  struct htype  *h   = set->data;
...
mtype_gc_init(struct ip_set *set, void (*gc)(unsigned long ul_set))
{
  ...
  h->wq.handle = create_workqueue(MTYPE_NAME);
  h->wq.set = set;
  INIT_WORK(&h->wq.work, mtype_deferred_gc);

	init_timer(&h->gc);
	h->gc.data = (unsigned long) set;
	h->gc.function = gc;
	h->gc.expires = jiffies + msecs_to_jiffies(IPSET_GC_PERIOD(set->timeout) * 100);
	add_timer(&h->gc);
...
mtype_expire(struct ip_set *set, struct htype *h, u8 nets_length, size_t dsize)
{
	...
	for (i = 0; i < jhash_size(t->htable_bits); i++) {
        ...
        mtype_timeout_notify(set->name, data);
...
mtype_gc(unsigned long ul_set)
{
	...
	pr_debug("called\n");
  queue_work(h->wq.handle, &h->wq.work);

	h->gc.expires = jiffies + msecs_to_jiffies(IPSET_GC_PERIOD(set->timeout) * 1000);
	add_timer(&h->gc);
}
...
out:
  if (ret && SET_WITH_TIMEOUT(set)) {
    u32 t = data->timeout ? data->timeout : ext->timeout;
    ip_set_timeout_set (ext_timeout(data, set), t);
  }
